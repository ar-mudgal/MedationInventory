...
public class MeditionServiceImpl implements MeditionService {
    ...
    @Override
    public Response addaddMeditionToBegB(MeditionListDto meditionListto) {
        ...
        for (MeditionRequest medition : meditionListto.getMedAndQuantityList()) {
            ...
            if(meditions.isPresent() || !meditions.isEmpty()) {
        ...
        return new Response("SUCCESS",addBags,HttpStatus.OK);

    }

    @Override
    public Response removeMeditionFromBag(){
        addBagRepository.deleteAll();
        return new Response("medition deleted successfully from bag",HttpStatus.OK);
    }
